    ...
    <svg width="0" height="0" style={{ position: 'absolute', pointerEvents: 'none' }} aria-hidden="true">
      <defs>
        <filter id={ROUGH_EDGE_ID} x="-3%" y="-3%" width="106%" height="106%">
          <feTurbulence type="fractalNoise" baseFrequency="0.035" numOctaves="4" stitchTiles="stitch" result="edgeNoise" />
          <feDisplacementMap in="SourceGraphic" in2="edgeNoise" scale="5" xChannelSelector="R" yChannelSelector="G" result="roughShape" />
          <feGaussianBlur in="roughShape" stdDeviation="0.2" />
        </filter>
...
// ── Spine-side page curve with ridge highlight ──
// Simulates the page bending into the binding:
// flat page → slight rise (ridge) → curve down into spine shadow
...
function SpineCurve({ side }: { side: 'left' | 'right' }) {
  ...
  return (
    <div className="absolute top-0 bottom-0 pointer-events-none select-none" style={{
      ...
      // Multi-stop gradient: flat → subtle highlight (paper rises) → shadow (paper curves down into binding)
      background: isLeft
...
// ── Top & bottom edge curve near spine ──
// Real book pages bow slightly near the binding at the top/bottom edges
...
// ── Page outer edge thickness ──
// Subtle highlight along the outer edge to give the page "depth"
...
function CornerCurl({ corner }: { corner: 'tl' | 'tr' | 'bl' | 'br' }) {
  const style: React.CSSProperties = { position: 'absolute', pointerEvents: 'none', zIndex: 3, width: 'clamp(35px, 7vw, 65px)', height: 'clamp(35px, 7vw, 65px)' };
  if (corner === 'tl') { style.top = 0; style.left = 0; style.background = 'radial-gradient(ellipse at 0% 0%, rgba(139,119,90,0.09) 0%, rgba(139,119,90,0.03) 40%, transparent 70%)'; }
  if (corner === 'tr') { style.top = 0; style.right = 0; style.background = 'radial-gradient(ellipse at 100% 0%, rgba(139,119,90,0.11) 0%, rgba(139,119,90,0.04) 40%, transparent 70%)'; }
  if (corner === 'bl') { style.bottom = 0; style.left = 0; style.background = 'radial-gradient(ellipse at 0% 100%, rgba(139,119,90,0.10) 0%, rgba(139,119,90,0.03) 40%, transparent 70%)'; }
  if (corner === 'br') { style.bottom = 0; style.right = 0; style.background = 'radial-gradient(ellipse at 100% 100%, rgba(139,119,90,0.08) 0%, rgba(139,119,90,0.03) 40%, transparent 70%)'; }
...
// ════════════════════════════════════
// RIGHT PAGE DECORATIONS (sparser)
// ════════════════════════════════════
...
function PagePaper({ side }: { side: 'left' | 'right' }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" style={{
      ...
      // Edge darkening — asymmetric per side
      boxShadow: isLeft
...
// ════════════════════════════════════
// SPINE — thick binding with depression
// ════════════════════════════════════
...
function Spine() {
  return (
    <div className="relative shrink-0 pointer-events-none select-none"
      ...
      {/* Deep fold depression — asymmetric gradient */}
      <div className="absolute inset-0" style={{
        background: `
          linear-gradient(90deg,
            rgba(105,75,45,0.28) 0%,
            rgba(95,65,38,0.16) 16%,
            rgba(90,60,35,0.06) 36%,
            rgba(90,60,35,0.01) 50%,
            rgba(90,60,35,0.05) 64%,
            rgba(95,65,38,0.14) 84%,
            rgba(100,75,45,0.24) 100%
          )
        `,
      }} />
      {/* Ridge highlights — asymmetric positions */}
      ...
      {/* Main binding stitch — thicker */}
      <div className="absolute left-1/2 top-[3%] bottom-[3%]" style={{
        width: '3px',
        background: 'repeating-linear-gradient(rgba(65,40,20,0.22) 0px, rgba(65,40,20,0.22) 6px, transparent 6px, transparent 15px)',
      }} />
      {/* Offset stitches */}
      <div className="absolute top-[3%] bottom-[3%]" style={{ left: '26%', width: '1.5px',
        background: 'repeating-linear-gradient(rgba(65,40,20,0.14) 0px, rgba(65,40,20,0.14) 4px, transparent 4px, transparent 12px)' }} />
      <div className="absolute top-[3%] bottom-[3%]" style={{ right: '34%', width: '1.5px',
        background: 'repeating-linear-gradient(rgba(65,40,20,0.10) 0px, rgba(65,40,20,0.10) 3px, transparent 3px, transparent 11px)' }} />
    </div>
  );
}
...
function tapePosition(seed: number): { pos: TapePos; rotation: number } {
  ...
  return { pos: positions[seed % positions.length], rotation: (seed * 13) % 16 - 8 };
}
...
function TapePiece({ seed }: { seed: number }) {
  ...
  const baseStyle: React.CSSProperties = {
    position: 'absolute', zIndex: 10, borderRadius: '1px', width: `${width}px`, height: `${10 + (seed % 6)}px`,
  ...
  return (
    <motion.div className="absolute group" style={{
      ...
      <div className="rounded-sm flex flex-col cursor-pointer" style={{
        width: 'clamp(130px, 16vw, 190px)', padding: 'clamp(5px, 0.7vw, 9px)', paddingBottom: 'clamp(24px, 3vw, 32px)',
...
export default function PolaroidScatter({ photos }: { photos: PolaroidPhoto[] }) {
  ...
  return (
    <>
      <AllSVGFilters />
      {/* Outer wrapper: global shadow + perspective + rough edge */}
      <div style={{
        // Perspective: stronger 3D tilt — book on a desk, viewed from slight angle
        transform: 'perspective(900px) rotateX(-2.5deg) rotateY(-0.8deg)',
        transformOrigin: 'center center',
        // Soft, large global shadow — "floating above background"
        filter: `url(#${ROUGH_EDGE_ID}) drop-shadow(10px 14px 32px rgba(0,0,0,0.13)) drop-shadow(18px 22px 64px rgba(0,0,0,0.07)) drop-shadow(4px 6px 10px rgba(0,0,0,0.06))`,
      }}>
        <div className="flex overflow-hidden" style={{
          minHeight: `${minHeight}px`, borderRadius: '8px',
        }}>
